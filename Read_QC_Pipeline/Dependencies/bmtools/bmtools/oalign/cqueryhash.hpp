...
public:
    ...
    void SetHashWordBitmask( CBitmaskAccessExt * wbm ) { m_wbmAccess = wbm; }
    ...
    CBitmaskAccessExt * GetHashWordBitmask() const { return m_wbmAccess; }
    ...
    enum EWindowGravity { eGravity_5prime, eGravity_3prime };
    int AddQuery( CQuery * query, EWindowGravity = eGravity_5prime );
...
protected:
    ...
    int  x_AddQuery( CQuery * query, int component, EWindowGravity g );
...
inline int CQueryHash::AddQuery( CQuery * query, EWindowGravity g )
{
    if( m_status == eStatus_empty ) CheckWordConstraints();
    
    m_status = eStatus_dirty;
    int ret = x_AddQuery( query, 0, g );
    if( query->HasComponent( 1 ) ) ret += x_AddQuery( query, 1, g );
