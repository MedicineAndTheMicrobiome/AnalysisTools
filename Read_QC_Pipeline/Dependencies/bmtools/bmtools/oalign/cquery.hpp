...
public:
    ...
    enum EFlags {
        ...
        fStoresInsertRange = 0x400,
        fNONE = 0
    };
    static Uint4 GetCount() { return s_count; }

    typedef pair<short,short> TSizeRange;
    
    // STS constructor
    CQuery( const string& id, const string& data1, const string& data2, const TSizeRange& szRange );
    ...
    void SetSequenceData( int component, const string& data, bool reverse );
    ...
    bool HasInsertRange() const { return m_flags & fStoresInsertRange; }
    TSizeRange GetInsertRange() const { return *(const TSizeRange*)m_data; }


    const char* GetId() const { return (const char*)m_data + (HasInsertRange()?sizeof(TSizeRange):0); }
...
inline void CQuery::SetSequenceData( int component, const string& data, bool reverse )
{
    if( data.length() == GetLength( component ) ) return;
    string x;
    if( reverse ) {
        switch( GetCoding() ) {
        case CSeqCoding::eCoding_ncbi8na:
            x.assign( data.length(), 'N' );
            for( unsigned i = 0; i < data.length(); ++i ) {
                x[data.length() - i - 1] = CIupacnaBase( data[i] ).Complement();
            }
            break;
        case CSeqCoding::eCoding_colorsp:
            x.reserve( data.length() );
            copy( data.rbegin(), data.rend(), back_inserter( x ) );
            break;
        default: THROW( runtime_error, "Only -q0 is allowed when using guide file as the sequence data source" );
        } 
    } else x = data;

    if( component == 0 ) {
        ostringstream mate;
        if( GetCoding() == CSeqCoding::eCoding_ncbi8na ) {
            PrintSeq<CNcbi8naBase>( mate, 1, CSeqCoding::eStrand_pos );
            x_InitNcbi8na( GetId(), x, mate.str(), kDefaultQualityBase );
        } else {
            PrintSeq<CColorTwoBase>( mate, 1, CSeqCoding::eStrand_pos );
            x_InitColorspace( GetId(), x, mate.str(), kDefaultQualityBase );
        }
    } else {
        ostringstream mate;
        if( GetCoding() == CSeqCoding::eCoding_ncbi8na ) {
            PrintSeq<CNcbi8naBase>( mate, 0, CSeqCoding::eStrand_pos );
            x_InitNcbi8na( GetId(), mate.str(), x, kDefaultQualityBase );
        } else {
            PrintSeq<CColorTwoBase>( mate, 0, CSeqCoding::eStrand_pos );
            x_InitColorspace( GetId(), mate.str(), x, kDefaultQualityBase );
        }
    }
}

inline CQuery::CQuery( const string& id, const string& data1, const string& data2, const TSizeRange& rsz ) 
    : m_data(0), m_flags(0)
{
    m_topHit[0] = m_topHit[1] = m_topHit[2] = 0;
    m_flags &= ~fCoding_BITS;
    m_flags |= fStoresInsertRange;
    ...
    unsigned sz = id.length() + m_length[0] + m_length[1] + 1 + sizeof( rsz );
    delete [] m_data;
    m_data = new unsigned char[sz];
    memset( m_data, 0, sz );
    *(TSizeRange*)m_data = rsz;
    const unsigned char * x = m_data + sizeof( rsz );
    strcpy( (char*)x, id.c_str() );
    m_offset[0] = sizeof( rsz ) + id.length() + 1;
