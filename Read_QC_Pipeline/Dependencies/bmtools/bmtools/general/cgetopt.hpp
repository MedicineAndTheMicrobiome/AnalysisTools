...
public:
    ...
    bool Handle( const char * arg ) { m_cnt++; *m_target = FromStr<T>( arg ); return true; }
...
public:
    ...
    void PrintValue( ostream& o ) const { o << *m_target?"true":"false"; }
protected:
    bool * m_target;
    int m_cnt;
};
...
public:
    COptArg( unsigned& tgt, char opt, const string& longOpt, const string& help ) :
        COption( eValueRequired, opt, longOpt, help ), m_target( &tgt ), m_base(10), m_cnt(0) {}
    bool Handle( const char * arg ) { 
        m_cnt++; 
        char * oops = 0;
        if( arg[0] == '0' && arg[1] ) {
            if( arg[1] == 'x' || arg[1] == 'X' ) *m_target = strtoul( arg + 2, &oops, m_base = 16 );
            else if( arg[1] == 'b' || arg[1] == 'B' ) *m_target = strtoul( arg + 2, &oops, m_base = 2 );
            else *m_target = strtoul( arg + 1, 0, m_base = 8 );
        } else *m_target = strtoul( arg, &oops, m_base = 10 );
        if( oops == 0 || oops[0] != 0 ) THROW( runtime_error, "Failed to convert ``" << arg << "'' to unsigned int (" << arg[1] << ") (" << oops << ")" );
        return true;
    }
    void PrintValue( ostream& o ) const { o << ToStr( *m_target, m_base ); }
protected:
    unsigned * m_target;
    int m_base;
    int m_cnt;
...
public:
    ...
    bool Handle( const char * arg ) {
        if( const char * colon = std::strchr( arg, ':' ) ) {
            m_target->first = FromStr<T>( string( arg, colon ) );
            m_target->second = FromStr<T>( colon + 1 );
            if( m_target->first > m_target->second ) swap( m_target->first, m_target->second );
        } else {
            m_target->first = m_target->second = FromStr<T>( arg );
        }
        return true;
    }
...
public:
    ...
    bool Handle( const char * arg ) { 
        ...
        if( m_delim.length() == 0 ) m_target->insert( FromStrX<value_type>( arg ) ); 
        else Split( arg, m_delim, inserter( *m_target, m_target->end() ), FromStrX<value_type> );
        return true;
    }
    void PrintValue( ostream& o ) const {
...
public:
    ...
    bool Handle( const char * arg ) { 
        if( arg == 0 || *arg == 0 ) { m_target->clear(); return true; }
        if( m_delim.length() == 0 ) m_target->push_back( FromStrX<value_type>( arg ) ); 
        else Split( arg, m_delim, back_inserter( *m_target ), FromStrX<value_type> );
        return true;
    }
    void PrintValue( ostream& o ) const {
        for( typename TData::const_iterator t = GetTarget().begin(), T = GetTarget().end(); t != T; ++t ) {
...
{
    m_getOpt.SetDone( 0 );
    std::cout << m_synopsis << " v." << m_version;
    if( m_build.length() ) std::cout << " (" << m_build << ")";
    std::cout << "\n";
    return true;
}
