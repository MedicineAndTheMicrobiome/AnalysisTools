...
#define kPattern_ff 0xffffffffffffffffULL
    ...
    static Uint8 GetBits( const uchar * table, Uint8 bitoff, int bitcnt );
    static void SetBits( uchar * table, Uint8 bitoff, int bitcnt, Uint8 value );
protected:
#ifndef _WIN32
...
inline Uint CBitHacks::InsertBits( const Uint& value, int offset ) 
{
    ...
    Uint ubits( bits );
    Uint insert( ubits << ( bitsperpos * offset ) );
...
inline Uint8 CBitHacks::GetBits( const uchar * table, Uint8 bitoff, int bitcnt ) 
{
    const uchar * addr = table + (bitoff+bitcnt)/8;
    Uint8 mask = WordFootprint<Uint8>( bitcnt );
    Uint8 value = 0;
    if( bitcnt + (bitoff%8) <= 8 ) { // all fits in one byte
        int rsh = 8 - (bitcnt + (bitoff%8));
        value = (table[bitoff/8] >> rsh) & mask;
    } else {
        int tail = (bitoff + bitcnt)%8;
        if( tail ) {
            value |= ((*addr) >> (8-tail));
            mask >>= tail;
        }
        int byte = 0;
        while( uchar(mask) == 0xff ) {
            value |= (Uint8(*--addr) << (8*byte + tail));
            mask >>= 8;
            byte++;
        }
        if( mask ) {
            value |= Uint8(*--addr & mask) << (8*byte + tail);
        }
    }
    return value;
}

inline void CBitHacks::SetBits( uchar * table, Uint8 bitoff, int bitcnt, Uint8 value ) 
{
    uchar * addr = table + (bitoff+bitcnt)/8;
    Uint8 mask = WordFootprint<Uint8>( bitcnt );
    value &= mask;
    if( bitcnt + (bitoff%8) <= 8 ) { // all fits in one byte
        int lsh = 8 - (bitcnt + (bitoff%8));
        bitoff /= 8;
        mask <<= lsh;
        value <<= lsh;
        (table[bitoff] &= ~mask) |= value;
    } else {
        if( int tail = (bitoff + bitcnt)%8 ) {
            uchar m = (uchar(~uchar(0)) >> tail);
            *addr &= m;
            *addr |= uchar(value) << (8-tail); // & (uchar(~uchar(0)) >> (8 - tail));
            value >>= tail;
            mask >>= tail;
        }
        while( uchar(mask) == 0xff ) {
