...
public:
    ...
    typedef CXRange<int> TRange;
    typedef map<string,string> TTags;
    enum EParseFlags {
        fDontParseTags = 0x001,
        fDontParseMDZtag = 0x002,
        fParse_Default = 0
    };
    ...
    bool IsHitMateOf( const CSamRecord& other ) const { return !WhyHitNotMateOf( other ); }
    unsigned WhyHitNotMateOf( const CSamRecord& other ) const;

    void WriteAsSam( ostream& out, bool basic = false ) const;
    string ToString( bool basic = false ) const { ostringstream o; WriteAsSam( o, basic ); return o.str(); }

    CSamRecord();
    CSamRecord( const string& samline, int flags  = fParse_Default );
    ...
    void SetNull();
    void Init( const string& line, int flags = fParse_Default );
    ...
    void SetSingle() { m_flags &= ~(fPairedHit|fMateReverse); m_mpos = m_isize = 0; }
