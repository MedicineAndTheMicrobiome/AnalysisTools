...
public:
    ...
    typedef string TSeqVector;
#else
    typedef CBioseq::TId TSeqIds;
    typedef CSeqVector TSeqVector;
#endif
    class ICallback 
    {
    public:
...
#endif
    virtual void Process( const TSeqIds&, const TSeqVector&, int oid = -1 );
...
{
    size_t oid = 0;
    for( CFaReader reader( name ); reader.good(); reader.next() ) {
        TSeqIds ids;
        ids.push_back( reader.GetIds() );
        Process( ids, reader.GetContents(), ++oid );
    }
}
#else
    ...
    if( gilist ) {
        ...
        for( TSeqIds::const_iterator i = ids.begin(); i != ids.end(); ++i ) {
            if( gilist->FindId(**i) ) { 
				...
				if( oid == -1 ) {
                    if( (*i)->IsGi() ) {
					    if( !gilist->GiToOid( (*i)->GetGi(), oid ) ) oid = -1;
                    } else if( (*i)->IsGeneral() && (*i)->GetGeneral().GetDb() == "ti") {
                        const CObject_id & obj = (*i)->GetGeneral().GetTag();
                        Int8 ti = (obj.IsId() ? obj.GetId() : NStr::StringToInt8(obj.GetStr()));
                        if( !gilist->TiToOid( ti, oid ) ) oid = -1;
                    } else {
                        Int8 num_id;
                        string str_id;
                        bool simpler;
                        SeqDB_SimplifySeqid( *const_cast<CSeq_id*>(&**i), 0, num_id, str_id, simpler);
                        if( !gilist->SiToOid( str_id, oid ) ) {
                            size_t pos = str_id.find(".");
                            if( pos != str_id.npos ) {
                                string nover( str_id, 0, pos );
                                if( !gilist->SiToOid( nover, oid ) ) {
                                    oid = -1;
                                }
                            }
                        }
...
inline void CSeqVecProcessor::Process( const TSeqIds& seqIds, const TSeqVector& vect, int oid )
{
