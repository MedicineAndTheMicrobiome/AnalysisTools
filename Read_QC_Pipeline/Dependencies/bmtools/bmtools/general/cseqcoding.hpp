...
inline string Ncbi8naToIupac( const char * a, const char * A, int extra = 0 ) 
{
    string out;
    if( a < A ) {
        a -= extra;
        ...
        out.reserve( A - a + 1 );
        out += "+";
        while( a <= A ) out += CIupacnaBase( CNcbi8naBase( a++ ) );
    } else {
        A -= extra;
        a += extra;
        out.reserve( a - A + 1 + 2*extra );
        out += "-";
        while( a >= A ) out += CIupacnaBase( CNcbi8naBase( a-- ).Complement() );
    }
    if( extra ) {
        for( size_t x = 0, y = out.size() - 1; x < extra; ++x, --y ) {
            out[x] = tolower( out[x] );
            out[y] = tolower( out[y] );
        }
    }
...
inline char * ReverseComplement( char * c, int cnt ) 
{
    ...
    if( i == j ) c[ i ] = Base( c + j ).Complement();
    return c;
}
