...
enum ETrimFlags { eTrim_left = 1, eTrim_right = 2, eTrim_both = 3 };
inline string Trim( const string& in, int flags = eTrim_both )
{
    size_t b = 0, e = in.length();
    while( b < e && isspace( in[b] ) ) ++b;
    while( e > b && isspace( in[e-1] ) ) --e;
    return in.substr( b, e-b );
}
inline string LTrim( const string& in ) { return Trim( in, eTrim_left ); }
inline string RTrim( const string& in ) { return Trim( in, eTrim_right ); }
...
template<> inline bool FromStr<bool>( const char * str, EOnError e ) 
{ 
    ...
    if( e == eOnError_ignore ) return false;
    THROW( invalid_argument, "bool from \"" << str << "\"" );
}
typedef pair<int,int> intpair_t;
template<> inline intpair_t FromStr<intpair_t>( const char * str, EOnError e )
{
    char * x = 0;
    int from = std::strtol( str, &x, 10 );
    int to = from;
    const char * delim = ":/-";
    if( x == 0 || (*x && strchr( delim, *x ) == 0 ) ) {
        if( e == eOnError_throw )
            THROW( invalid_argument, "pair<int,int> from \"" << str << "\"" );
        else return make_pair( from, to );
    }
    if( *x == 0 ) return make_pair( from, to );
    to = strtol( x, &x, 10 );
    if( x == 0 || *x ) {
        if( e == eOnError_throw )
            THROW( invalid_argument, "pair<int,int> from \"" << str << "\" delimited with `" << delim << "'" );
        to = from;
    }
...
{ return Split( str, delim, i, FromStrX<const char*>, tokenize ); }
