...
CSamRecord::CSamRecord( const string& line, int parse_flags ) :
    m_qname(new char[1]),
    m_rname(m_qname),
    m_qiupac(m_qname),
    m_qqual(m_qname),
...
void CSamRecord::Init( const string& line, int parse_flags )
{
    SetNull();
    ...
    m_flags = FromStr<int>( fields[eCol_flags] );
    m_pos = FromStr<int>( fields[eCol_pos] ); 
    m_mapq = FromStr<int>( fields[eCol_mapq] );
    m_mpos = FromStr<int>( fields[eCol_mpos] );
    m_isize = FromStr<int>( fields[eCol_isize] );
    ...
    string mdtag = "";
    if( (parse_flags & fDontParseTags) == 0 ) {
        ...
        for( size_t i = eCol_tags; i < fields.size(); ++i ) {
            ...
            (*m_tags)[tag.substr( 0, x )] = tag.substr( x + 1 );
        }
        if( ((parse_flags & fDontParseMDZtag) == 0) && cigar != "*" && mdtag != "" ) {
#if 1            
            TCigar fullcigar;
            ParseSamMdTag( fullcigar, TCigar( cigar ), mdtag.c_str() );
            m_cigar = fullcigar;
#else
            TCigar c( cigar );
...
{
    return FromStr<int>( GetTagString( name ) );
}

float CSamRecord::GetTagFloat( const string& name ) const
{
    return FromStr<double>( GetTagString( name ) );
}
...
{
    return TRange::MakeOpen( m_pos, m_pos + m_cigar.ComputeLengths().second );
}
...
unsigned CSamRecord::WhyHitNotMateOf( const CSamRecord& other ) const
{
    if( IsNull() ) return 0x0001;
    if( !IsPairedHit() ) return 0x0002;
    if( !other.IsPairedHit() ) return 0x0004;
    if( m_flags & fSeqIsFirst && other.m_flags & fSeqIsFirst ) return 0x0010;
    if( m_flags & fSeqIsSecond && other.m_flags & fSeqIsSecond ) return 0x0020;
    if( strcmp( m_qname, other.m_qname ) ) return 0x0040;
    if( strcmp( m_rname, other.m_rname ) ) return 0x0080;
    /*
    if( IsReverse() != bool( other.m_flags & fMateReverse ) ) return false;
    if( other.IsReverse() != bool( m_flags & fMateReverse ) ) return false;
    */
    if( m_pos != other.m_mpos ) return 0x0100;
    if( m_mpos != other.m_pos ) return 0x0200;
    if( m_isize != -other.m_isize ) return 0x0400;
    // TODO: check isize (???)
    return 0;
}
...
CSamMdParser::TCigar CSamMdParser::GetFullCigar()
{
    ...
    TCigar fullcigar;
    ParseSamMdTag( fullcigar, m_cigar, m_mdtag.c_str() );
    ...
    //DEBUG( "DONE: " << DISPLAY( m_cigar.ToString() ) << DISPLAY( m_mdtag ) << DISPLAY( fullcigar.ToString() ) );
#endif
    return fullcigar;
};
