...
usage= paste(
	...
	"	[-w <paper width>]\n",
	"\n",
	"This script will plot the CDF curves for the non-core taxa.\n",
	"\n",
	"The non-core are:\n",
	"	Low Abundance but High Ubiquity\n",
	"	High Abundance but Low Ubiquity\n",
	"\n",
	"\n", sep="");
...
OutputFileRoot=paste(OutputFileRoot, ".nonCore", sep="");
...
# Plot the CDF
	...
	if(num_taxa>0){
		...
		for(t in draw_order){
			lines(log_cutoffs, cdf_info$cdf_val[t,], col=cdf_info$colors[t], 
