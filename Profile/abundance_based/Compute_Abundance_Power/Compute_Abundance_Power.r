...
usage = paste(
	...
	"\n",
	"The target category could be something like:\n",
...
if(!length(opt$input_file) || !length(opt$target_category)){
	cat(usage);
	q(status=-1);
}

InputFileName=opt$input_file;
TargetedCategory=opt$target_category;
...
cat("Target Category: ", TargetedCategory, "\n");
...
targeted_abundance=normalized_mat[,TargetedCategory];

pdf(paste(OutputFileRoot,".abundance_power.pdf", sep=""));

hist(targeted_abundance, breaks=20,
	xlab="Relative Abundance",
	main=paste(TargetedCategory, ": Relative Abundance", sep=""));
hist(log(targeted_abundance/(1-targeted_abundance)), breaks=20,
	xlab="Logit",
	main=paste(TargetedCategory, ": Logit", sep=""));

###############################################################################


# Determine which effect sizes to try
range=range(targeted_abundance);
span=diff(range);
effect_sizes=seq(span/40, span, length.out=40);
setdiff(effect_sizes, 0);

cat("The range of abundances: ", range[1], "-", range[2], "\n");
cat("The span: ", span, "\n");
cat("\n");
cat("Computing power over effect sizes: \n");
print(effect_sizes);
cat("\n");
...
ci95=quantile(targeted_abundance, c(.025, .975));
cat( "N:", length(targeted_abundance), "\n",
	"Mean:", mean(targeted_abundance), "\n",
	"Stdev:", sd(targeted_abundance), "\n",
	"Median:", median(targeted_abundance), "\n",
	"95% CI:(", ci95[1], ",", ci95[2], ")\n",
	"\n",
	sep=""
);
...
out_fh=file(paste(OutputFileRoot, ".abundance_power.csv", sep=""), "w");
cat(file=out_fh,
	"N:,", length(targeted_abundance), "\n",
	"Mean:,", mean(targeted_abundance), "\n",
	"Stdev:,", sd(targeted_abundance), "\n",
	"Median:,", median(targeted_abundance), "\n",
	"95% CI:(,", ci95[1], ",", ci95[2], ",)\n",
...
for(effect_size in effect_sizes){
	...
	samp1=targeted_abundance;
	samp2=targeted_abundance+effect_size;
	comb=c(samp1, samp2);
	...
	cat("Abundance Effect Size: ", effect_size, "  Cohen's Eta^2: ", r_sqrd, "\n");
	...
	for(N1 in N1_range){
		...
		beta_at_alpha=compute_beta(targeted_abundance, effect_size, N1, N2, Alpha);
