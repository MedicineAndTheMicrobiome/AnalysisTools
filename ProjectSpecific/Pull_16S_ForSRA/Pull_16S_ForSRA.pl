...
my %control_hash;
$control_hash{"0000"}="Extraction_Kit_Negative_Control#kit_reagents";
$control_hash{"0001"}="PCR_Amplification_Negative_Control#pcr_reagents";
$control_hash{"0002"}="PCR_Amplification_PositiveZymo_Control#control_DNA";
...
for(my $i=0; $i<$num_entries; $i++){
	...
	# Parse sample types
	my $sample_type=$sample_type_hash{$sample_name_toks[4]};
	my @sample_type_tok=split "#", $sample_type;
	my $tissue=$sample_type_tok[0];
	my $host=$sample_type_tok[1];
	my $medium=$medium_hash{$tissue};
	my $local_scale=$local_scale_hash{$tissue};
	...
	if(defined($control_info)){
		my ($control_name, $control_scale)=split "#", $control_info;
		$sample_name=$control_name;
		$local_scale=$control_scale;
		$medium=$control_scale;
	...
	my $outstr=join "\t", ($sample_name, $library_id, $title, $sample_date_form, $local_scale, $medium, $host_subject_id1, $host_subject_id2, $f1_list[$i], $f2_list[$i]);
