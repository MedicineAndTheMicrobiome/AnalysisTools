...
usage = paste(
	...
	"	-p <target parent GO ID>\n",
	"	-o <output directory>\n",
	"\n",
	"This script will:\n",
	"  1.) Take the specified GO parent ID and look up it's immediate children. \n",
	...
	"\n",
	"Both of these files will be placed into the specified output directory.\n",
	"\n");

if(
	!length(opt$input) |
	!length(opt$parent_id) |
...
cln_pid=gsub(":", "_", ParentID);

cat("\n");
cat("Targeted Parent: ", ParentID, "\n");
cat("OBO Map Name: ", InputOBOMap, "\n");
cat("Output Dir: ", OutputDir, "\n");
cat("\n");
cat("Clean Parent ID: ", cln_pid, "\n");
cat("\n");
...
parent_id_name=lookup_name(ParentID, id_to_name_map);
cat("\n\n");
cat("Name of Parent ID: ", ParentID, ":\n", sep="");
cat("\"", parent_id_name, "\"\n", sep="");
cat("\n");

immed_children_arr=find_immediate_children(ParentID, go_tree);
