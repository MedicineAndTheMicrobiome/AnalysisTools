...
#                list=$in.unique.good.filter.unique.precluster.pick.opti_mcc.list,
	...
	##############################################################################
	# Handle representative sequences + filtering (≥10) + distance matrix
	my $Skip_OTU_RepresentativeSequence=0;
	if(!$Skip_OTU_RepresentativeSequence){

		my $list_file  = "$in.unique.good.filter.unique.precluster.pick.opti_mcc.$clust_cutoff.list";
		my $count_file = "$in.unique.good.filter.unique.precluster.pick.count_table";
		my $fasta_file = "$in.unique.good.filter.unique.precluster.pick.fasta";
		my $min_otu_size = 10; # Minimum number of sequences required to retain an OTU

		# Step 1 — get representative sequences (all OTUs)
		execute_mothur_cmd(
			"get.oturep",
			"list=$list_file,
			fasta=$fasta_file,
		...
		# Output: *.opti_mcc.$clust_cutoff.$clust_cutoff.rep.fasta

		# Step 2 — get OTU abundance distribution
		execute_mothur_cmd(
			"get.rabund",
			"list=$list_file,
			count=$count_file"
		);
		# Output: *.opti_mcc.$clust_cutoff.rabund

		# Step 3 — filter OTUs with ≥10 sequences
		execute_mothur_cmd(
			"split.abund",
			"list=$list_file,
			count=$count_file,
		...
		# Output: *.opti_mcc.$clust_cutoff.$clust_cutoff.abund.list

		# Step 4 — representative sequences for abundant OTUs only
		my $abund_list = 
			"$in.unique.good.filter.unique.precluster.pick.opti_mcc.$clust_cutoff.$clust_cutoff.abund.list";
		...
		# Output: *.abund.$clust_cutoff.rep.fasta

		# This should also be the file used if we want to do Kraken for Species level
		my $rep_fasta = 
			"$in.unique.good.filter.unique.precluster.pick.opti_mcc.$clust_cutoff.$clust_cutoff.abund.$clust_cutoff.rep.fasta";

		##############################################################################
		# Step 5 — compute OTU distance matrix ("OTU embeddings") (use original IDs first)
		execute_mothur_cmd(
			"dist.seqs",
			"fasta=$rep_fasta,
			cutoff=1.00,
		...
		my $dist_file = $rep_fasta;
		$dist_file =~ s/\.fasta$/.dist/;

		##############################################################################
		# Step 6a — generate seq_to_otu mapping (from abund.list)

		##############################################################################
		# Generate sequence-to-OTU mapping from an abund.list file
		#
		# Input:
		#   $abund_list - mothur .abund.list file
		#   $map_file   - output mapping file
		#
		# Output:
		...
		my $map_file = "$in.seq_to_otu.tsv";

		generate_seq_to_otu_mapping($abund_list, $map_file);

		sub generate_seq_to_otu_mapping {
			# The sequence ID is based on the sample name, but the OTU number assignment
			# is made after abundance has been calculated.  We only want to output in OTU number
			# space.

			my ($abund_list, $map_file) = @_;

			open(my $LIST, "<", $abund_list) or die "Cannot open $abund_list: $!";
			open(my $MAP, ">", $map_file) or die "Cannot write $map_file: $!";

			# Read header
			my $header = <$LIST>;
			chomp $header;

			my @otu_names = split(/\t/, $header);

			# Remove label and numOtus columns
			shift @otu_names;
			shift @otu_names;

			# Process each clustering row
			while (<$LIST>) {
				...
				for (my $i = 0; $i < @cols; $i++) {
					...
					my $otu_name = $otu_names[$i];
					my $seqs     = $cols[$i];

					next unless defined $seqs && $seqs ne "";

					# Output all the sequence-to-OTU name mappings
					# even though only one of them will be selected
					# as the representative sequence
					foreach my $seq (split(/,/, $seqs)) {
						print $MAP "$seq\t$otu_name\n";
					}
				}
			}
		...
		##############################################################################
		# Step 6b Rename representative FASTA sequence IDs to OTU IDs
		#
		# Input:
		#   $rep_fasta - representative FASTA generated by get.oturep
		#   $map_file  - sequence-to-OTU mapping file
		#   $fasta_out - output FASTA with OTU IDs
		#
		# Output:
		#   Representative FASTA with OTU IDs as sequence headers
		##############################################################################

		my $fasta_out = $rep_fasta;
		$fasta_out =~ s/\.rep\.fasta$/\.rep_OTU.fasta/;

		rename_rep_fasta_to_otu_ids(
			$rep_fasta,
			$map_file,
			$fasta_out
		);

		sub rename_rep_fasta_to_otu_ids {

			my ($rep_fasta, $map_file, $fasta_out) = @_;

			# Read sequence-to-OTU mapping
			my %map;

			open(my $MAPIN, "<", $map_file) or die "Cannot open $map_file: $!";
			...
			open(my $IN, "<", $rep_fasta) or die "Cannot open $rep_fasta: $!";
			open(my $OUT, ">", $fasta_out) or die "Cannot write $fasta_out: $!";

			while (<$IN>) {

				if (/^>(\S+)/) {
					my $seqid = $1;
					my $newid = exists $map{$seqid} ? $map{$seqid} : $seqid;
					print $OUT ">$newid\n";
				} else {
					print $OUT $_;
				}
		...
		##############################################################################
		# Step 6c: Rename representative distance matrix sequence IDs to OTU IDs
		#
		# Input:
		#   $dist_file - representative distance matrix (.rep.dist)
		#   $map_file  - sequence-to-OTU mapping file
		#   $dist_out  - output distance matrix with OTU IDs
		#
		# Output:
		#   Representative distance matrix with OTU IDs
		##############################################################################

		my $dist_out = $dist_file;
		$dist_out =~ s/\.rep\.dist$/\.rep_OTU.dist/;

		rename_distance_matrix_to_otu_ids(
			$dist_file,
			$map_file,
			$dist_out
		);

		print "Done. OTU-labeled FASTA: $fasta_out\n";
		print "Done. OTU-labeled distance matrix: $dist_out\n";

		sub rename_distance_matrix_to_otu_ids {

			my ($dist_file, $map_file, $dist_out) = @_;
			...
			# Read in sequence ID distmat and convert IDs to OTUs
			open(my $DISTIN, "<", $dist_file) or die "Cannot open $dist_file: $!";
			open(my $DISTOUT, ">", $dist_out) or die "Cannot write $dist_out: $!";

			while (<$DISTIN>) {
				# \b ... \b  Word boundaries
				# \Q ... \E  Quoting pattern literally

				foreach my $seq (keys %map) {
					s/\b\Q$seq\E\b/$map{$seq}/g;
				}
				print $DISTOUT $_;
			}
	...
	##############################################################################
	# End of OTU Representative Sequence steps
