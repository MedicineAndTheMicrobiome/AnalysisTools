...
DateFormatConversionDialogBox=function(in_date, in_varname){
	...
	modalDialog(
		fluidPage(
			 ...
			 fluidRow(
				column(4,
					selectInput("DFCD.input_dates_select", 
				...
				column(4,
					selectInput("DFCD.date_format_select", 
				...
				column(4,
					selectInput("DFCD.converted_dates_select", 
		...
		footer=fluidRow(
				column(1, actionButton("DFCD.helpButton", label="Help")),
				column(7),
				column(2, actionButton("DFCD.saveAsButton", label="Save As...")),
				column(2, actionButton("DFCD.cancelButton", label="Cancel"))
			)
		
	)
}

RenameDialogBox=function(old_name, mesg=NULL){
	modalDialog(
		fluidPage(
			ifelse(is.null(mesg), "", tags$h4(mesg)),
			tags$b("Current Variable Name"),
			tags$h5(old_name),
			textInput("ReNmDB.new_variable_name", "New Variable Name", value=paste(old_name, "_isodate", sep="")),
		),
		footer=fluidRow(
...
observe_DateFormatConversionDialogBoxEvents=function(input, output, session, current_variable_names){
